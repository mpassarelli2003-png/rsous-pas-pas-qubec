...
const HELP_CONTENT: Record<string, { title: string; steps: string[]; example: string[]; tryText: string }> = {
  arithmetique: {
    ...
    steps: ['Je regarde les nombres importants.', 'Je choisis des nombres plus faciles à calculer.', 'Je fais un calcul rapide.', 'Je garde cette réponse pour vérifier si mon vrai résultat a du sens.'],
    example: ['Exemple avec d’autres nombres : 327 + 189', '327 ≈ 300 et 189 ≈ 200', '300 + 200 = 500', 'Mon estimation est environ 500.'],
    tryText: 'À ton tour : choisis des nombres faciles avec les nombres de ton problème.',
  },
  mesure: {
    title: 'Comment estimer en mesure',
    steps: ['Je regarde l’unité : cm, m, kg, L, minutes, degrés…', 'Je choisis une mesure proche et facile.', 'Je vérifie si l’unité de ma réponse est logique.'],
    example: ['Exemple avec d’autres nombres : une corde mesure 198 cm.', '198 cm est proche de 200 cm.', 'Mon estimation est environ 200 cm.'],
    tryText: 'À ton tour : garde la bonne unité et choisis une mesure proche.',
  },
  probabilite: {
    title: 'Comment estimer en probabilité',
    steps: ['Je compare les quantités possibles.', 'Je regarde ce qui est le plus nombreux, le moins nombreux ou impossible.', 'Je prévois ce qui est le plus probable sans faire un long calcul.'],
    example: ['Exemple avec d’autres nombres : 10 jetons verts et 3 jetons jaunes.', 'Il y a plus de jetons verts que de jetons jaunes.', 'J’estime qu’il est plus probable de piger un jeton vert.'],
    tryText: 'À ton tour : compare les quantités de ton problème.',
  },
  proportion: {
    title: 'Comment estimer avec une proportion',
    steps: ['Je repère la relation de départ.', 'Je cherche un multiplicateur facile.', 'Je prévois une réponse proche avant de calculer exactement.'],
    example: ['Exemple avec d’autres nombres : une recette pour 4 personnes doit servir 12 personnes.', '12 est 3 fois plus grand que 4.', 'J’estime qu’il faudra environ 3 fois chaque ingrédient.'],
...
export function Step4Plan({ problem, onUpdate, savedData, step3Data }: Step4PlanProps) {
  ...
  const pushUpdate = (patch: any) => {
    onUpdate({ selectedOps, estimation, whyOps, planRows, stepsCount: String(planRows.length), easyNumbers, estimationSteps, quickCalculation, ...patch });
  };
      ...
      <aside className="w-full lg:sticky lg:top-28 lg:self-start space-y-3 z-[1]">
        <div className="rounded-xl border border-blue-300 bg-blue-50 p-3 shadow-sm">
          ...
          <ol className="space-y-2 text-sm text-blue-950">
            {['Je regarde ce que je cherche.', 'Je regarde les infos utiles.', 'Je choisis l’opération.', 'Je prévois mes étapes.', 'J’estime avant de calculer.'].map((txt, i) => (
      ...
      <div className="min-w-0 space-y-10">
        <section className="space-y-4">
          <div className="space-y-1">
            <h3 className="text-xl font-bold flex items-center gap-2"><Calculator className="h-5 w-5 text-primary" />4A. Je choisis les opérations utiles</h3>
                ...
                <div key={op.id} className={cn('relative rounded-xl border-2 bg-white transition-all', selected ? cn(op.border, op.light, 'shadow-sm ring-2', op.ring) : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50')}>
                  ...
                  {helpOpen && (
                    <div className="mx-3 mb-3 rounded-xl border border-blue-200 bg-white p-3 shadow-sm text-sm text-slate-700">
                      <p className="font-bold text-slate-900 mb-1">Quand l’utiliser ?</p><p className="leading-snug">{op.definition}</p>
                      <p className="font-bold text-slate-900 mt-2 mb-1">Mots utiles</p><div className="flex flex-wrap gap-1">{[...op.keywords, ...op.moreKeywords].map(k => <span key={k} className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700">{k}</span>)}</div>
                      <p className="font-bold text-slate-900 mt-3 mb-1">Débuts de phrase</p><div className="flex flex-wrap gap-1">{op.starters.map(starter => <span key={starter} className="rounded-md border border-blue-100 bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-800">{starter}</span>)}</div>
                    </div>
                  )}
                </div>
              );
        ...
        <section className="space-y-6">
          <div className="space-y-2"><h3 className="text-xl font-bold flex items-center gap-2"><ListOrdered className="h-5 w-5 text-primary" />4B. Ma planification</h3><p className="text-muted-foreground text-sm italic">Combien d'étapes de calcul faudra-t-il ? Ajoute autant de lignes que nécessaire.</p></div>
        ...
        <section className="space-y-5">
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3 flex-wrap"><h3 className="text-xl font-bold flex items-center gap-2"><Target className="h-5 w-5 text-primary" />4C. Je fais une estimation</h3><Button variant="outline" size="sm" onClick={() => setShowEstimationHelp(true)} className="gap-2 border-blue-200 text-blue-800 hover:bg-blue-50"><HelpCircle className="h-4 w-4" /> Comment estimer ?</Button></div>
          ...
          <Card className="p-4 border-2 border-yellow-200 bg-yellow-50/70 space-y-4">
            ...
            {estimationType === 'probabilite' ? (
              <div className="grid md:grid-cols-2 gap-3"><div><label className="text-xs font-bold uppercase tracking-wide text-yellow-900">Je compare</label><Input value={estimationSteps[0] || ''} onFocus={() => setActiveEstimationStep(0)} onChange={e => updateEstimationStep(0, e.target.value)} placeholder="Ex: Il y a plus de... que de..." className="mt-1 bg-white" /></div><div><label className="text-xs font-bold uppercase tracking-wide text-yellow-900">Mon estimation</label><Input value={estimation} onChange={e => updateEstimation(e.target.value)} placeholder="Ex: Le plus probable est..." className="mt-1 bg-white" /></div></div>
            ) : (
              <>
                ...
                <div className="space-y-3"><div className="flex items-center justify-between gap-3 flex-wrap"><label className="text-xs font-bold uppercase tracking-wide text-yellow-900">Mon calcul rapide, en plusieurs étapes</label><Button type="button" variant="outline" size="sm" onClick={addEstimationStep} className="h-8 gap-1 bg-white border-yellow-300 text-yellow-900 hover:bg-yellow-100"><Plus className="h-4 w-4" /> Ajouter une étape</Button></div><div className="space-y-2">{estimationSteps.map((step, index) => <div key={index} className="grid grid-cols-[2rem_1fr_auto] items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-yellow-200 text-yellow-950 font-bold text-sm">{index + 1}</span><Input value={step} onFocus={() => setActiveEstimationStep(index)} onChange={e => updateEstimationStep(index, e.target.value)} placeholder={index === 0 ? 'Ex: nombre facile + nombre facile = total proche' : 'Ex: total proche + nombre facile = estimation'} className="bg-white" /><Button type="button" variant="ghost" size="icon" onClick={() => removeEstimationStep(index)} className="h-9 w-9 text-slate-500 hover:text-red-600 hover:bg-red-50" aria-label={`Supprimer l'étape d'estimation ${index + 1}`}><Trash2 className="h-4 w-4" /></Button></div>)}</div><div className="rounded-xl bg-white/80 border border-yellow-200 p-3"><p className="text-xs font-bold uppercase tracking-wide text-yellow-900 mb-2">Symboles utiles pour ton calcul rapide</p><div className="flex flex-wrap gap-2">{ESTIMATION_OPERATIONS.map(op => <button key={op.symbol} type="button" onClick={() => insertEstimationSymbol(op.symbol)} className="h-10 min-w-10 rounded-lg border border-yellow-300 bg-white px-3 text-lg font-black text-yellow-950 hover:bg-yellow-100" aria-label={`Insérer le symbole ${op.label}`}>{op.symbol}</button>)}</div></div></div>
                <div><label className="text-xs font-bold uppercase tracking-wide text-yellow-900">Mon estimation</label><Input value={estimation} onChange={e => updateEstimation(e.target.value)} placeholder="Ex: environ la réponse proche" className="mt-1 bg-white font-bold" /></div>
              </>
            )}
          </Card>
        </section>
      ...
      {showEstimationHelp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl border border-slate-200 overflow-hidden">
            <div className="bg-primary text-primary-foreground px-5 py-4 flex items-center justify-between gap-3"><h4 className="font-bold text-lg">{help.title}</h4><button type="button" onClick={() => setShowEstimationHelp(false)} className="rounded-full bg-white/15 hover:bg-white/25 h-8 w-8 flex items-center justify-center" aria-label="Fermer">×</button></div>
            <div className="p-5 space-y-4"><div className="rounded-xl bg-blue-50 border border-blue-100 p-3"><p className="font-bold text-blue-950 mb-2">Ce que je fais</p><ol className="space-y-1 text-sm text-blue-950 list-decimal list-inside">{help.steps.map(step => <li key={step}>{step}</li>)}</ol></div><div className="rounded-xl bg-yellow-50 border border-yellow-200 p-3"><p className="font-bold text-yellow-950 mb-2">Exemple avec d’autres nombres</p><div className="space-y-1 text-sm text-yellow-950">{help.example.map(line => <p key={line}>{line}</p>)}</div></div><p className="text-sm font-medium text-slate-700">{help.tryText}</p><div className="flex justify-end"><Button onClick={() => setShowEstimationHelp(false)}>Je comprends</Button></div></div>
          </div>
        </div>
      )}
    </div>
