        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 max-w-2xl mx-auto">
            <Button
              ...
              className="h-24 flex flex-col gap-1 rounded-2xl border-2"
              onClick={() => setLevel(5)}
            >
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  variant={!selectedSubConcept ? 'primary' : 'outline'}
                  size="sm"
                ...
                {visibleSubConcepts.map(item => (
                  <Button
                    key={item.id}
                    variant={selectedSubConcept === item.id ? 'primary' : 'outline'}
                    size="sm"
                    className="rounded-full"
                    onClick={() => setSelectedSubConcept(selectedSubConcept === item.id ? null : item.id)}
                  >
                    {item.label}
                  </Button>
                ))}
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto pr-1">
                <Button
                  variant={!selectedSocialContext ? 'primary' : 'outline'}
                  size="sm"
                  className="rounded-full"
                  onClick={() => setSelectedSocialContext(null)}
                >
                  Tous
                </Button>
                {SOCIAL_CONTEXTS.map(context => (
                  <Button
                    key={context}
                    variant={selectedSocialContext === context ? 'primary' : 'outline'}
                    size="sm"
                    className="rounded-full"
                    onClick={() => setSelectedSocialContext(selectedSocialContext === context ? null : context)}
                  >
                    {context}
                  </Button>
                ))}
              ...
              return (
                <Card key={problem.id} className="p-6 hover:shadow-lg transition-all border-2 group hover:border-primary/50 relative overflow-hidden">
                  ...
                  <div className="space-y-4">
                    ...
                    {socialContext && (
                      <p className="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1">
                        Univers social : {socialContext}
                      </p>
                    )}
